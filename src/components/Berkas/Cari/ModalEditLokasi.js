...
export default class ModalEdit extends Component {
	...
	editSubmit = async e => {
		...
		if(isSend){
			const body = {query: `
				mutation {
					berkas: editBerkas(id: "${formData.id}", update: {
						...
						${formData.nama_wp ? `nama_wp: "${formData.nama_wp}"` : ``}					
						...
						${formData.nama_penerima ? `nama_penerima: "${formData.nama_penerima}"` : ``}
						${formData.tgl_terima ? `tgl_terima: "${formData.tgl_terima}"` : ``}
			...
			return fetchDataGQL(body)
				...
				.then(({data}) => {
					this.closeModal()
					document.getElementById('cariBerkas').click()
				})
				.catch(err => { throw err })
		}
	}
	...
	componentWillReceiveProps(nextProps){
		if(this.props.berkas !== nextProps.berkas){
			this.setState({ formData: {
				...
				npwp: nextProps.berkas.pemilik ? nextProps.berkas.pemilik.npwp : null,
				nama_wp: nextProps.berkas.pemilik ? nextProps.berkas.pemilik.nama_wp : null,
				kd_berkas: nextProps.berkas.ket_berkas.kd_berkas,
				nama_penerima: nextProps.berkas.penerima ? nextProps.berkas.penerima.nama_penerima : null,
				tgl_terima: nextProps.berkas.penerima ? nextProps.berkas.penerima.tgl_terima : null,
			...
			}, disableNPWP: nextProps.berkas.pemilik ? false : true, disablePenerima: nextProps.berkas.penerima ? false : true })
		}
	}
	...
	render(){
		...
		return(
			<div className="modal fade" id="modalEditLokasi" tabIndex="-1" role="dialog">
									...
									<div className="form-group col-md-5">
										...
										<div className="input-group">
											<InputMask 
												...
												onChange={ this.changeHandler }
												disabled={ this.state.disableNPWP }
											/>
										</div>
									</div>
									<div className="form-group col-md-7">
